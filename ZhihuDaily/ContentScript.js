...
$(document).ready(function() {
    ...
    var headlineBackground = $('div.headline-background');
    if (!headlineBackground.length)
    $('.headline').css('border-bottom', '0px');
...
window.update = function(info) {
    ...
    if (info.theme_id && info.theme_name && info.theme_image) {
        $theme = $('<div class="origin-source-wrap"><a class="origin-source with-link" href="zhihudaily://theme/' + info.theme_id + '"><img src="' + info.theme_image + '" class="source-logo"><span class="text">本文来自：' + info.theme_name + '</span></a><a class="focus-link" href="javascript:;"><span class="btn-label">关注</span></a></div>').appendTo($(".question").last());
    } else if (info.section_id && info.section_name && info.section_thumbnail) {
        $(".question").last().append('<div class="origin-source-wrap unfocused"><a class="origin-source with-link" href="zhihudaily://section/' + info.section_id + '"><img src="' + info.section_thumbnail + '" class="source-logo"><span class="text">本文来自：' + info.section_name.replace(/#/g, "") + ' · 合集</span></a></div>');
    }
    if (info.theme_id) {
        setTimeout(function(){
            ...
            if (window.daily) {
                $theme.find('a.focus-link').on('click', function() {
                    ...
                    active_click = false;
                    shouldShowHUD = true;
                    window.daily.execCommand('themes/add ' + info.theme_id);
                    window.daily.sendGAEventWithCategoryActionLabel('Click', "Follow on Story Bottom", info.theme_name);
                });
                $theme.find('a.with-link').on('click', function() {
                    window.daily.sendGAEventWithCategoryActionLabel('Click', "Theme on Story Bottom", info.theme_name);
                    return true;
                });
                window.dailyHandleEvent = function(event) {
                    switch (event.eventName) {
                        case "theme action failed":
                            active_click = true;
                            if (event.command == 'themes/add ' + info.theme_id) {
                                if (shouldShowHUD) {
                                    shouldShowHUD = false;
                                    window.daily.showHUD('关注失败');
                                }
                            } else if (event.command == 'themes/remove ' + info.theme_id) {
                                shouldShowHUD = false;
                            }
                            break;
